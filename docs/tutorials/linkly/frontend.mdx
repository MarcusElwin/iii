---
title: "Ch. 6: Bring in the browser"
description: "Turn a browser tab into a iii worker that creates links, shows live clicks, and answers server-initiated prompts."
owner: "devrel"
type: "tutorial"
---
...
Open the component and declare its state — the form fields, the just-created link, and the live click
counter:
...
(`ui::on_click`) and a `stream` trigger that routes every new row to it; the cleanup unregisters both
on unmount:
...
[Ch. 7: Open it to users](/tutorials/linkly/custom-rules), customers attach per-link scripts that
