---
title: "A link-safety agent"
description: "Drop in an autonomous LLM agent that samples new links, investigates destinations in sandboxes, and quarantines or asks a human to confirm deletion."
owner: "devrel"
type: "tutorial"
---
...
Finally, subscribe to `link.created`:
...
worker.registerFunction('safety::on_link_created', async (data: { code: string; url: string }) => {
  ...
  await investigate(data).catch((e) => logger.warn('investigation failed', { code: data.code, error: String(e) }))
  return { sampled: true }
})
...
In production you sample at maybe 1% (`SAFETY_SAMPLE_RATE=0.01`); for this tutorial the default is 1
so every new link is investigated.
...
## Admin page (one-line addition to the frontend)

Chapter 6's browser worker already registers `user::confirm_destructive_op`, the function the
server calls when the agent picks `propose_delete`. You don't need a new component; the existing
`window.confirm` prompt is the admin surface. In a real product you'd swap that for a queue of
pending requests, but the seam is the same: the agent calls the function, the browser answers it.
