...
Linkly's links live in `iii-state`, which runs in-memory by default. Restart the engine and
...
<Info>
  If you prefer to keep `iii-state` as the only persistence layer rather than introducing a database,
  flip it to file-backed instead by setting `store_method: file_based` and a `file_path` in its
  config. The rest of this chapter assumes the database route.
</Info>
...
First, add the HTTP types you'll need shortly and name the database. Append the `ApiRequest` /
`ApiResponse` import to the existing `iii-sdk` import line, and add the `DB` constant near the top
of the file:
