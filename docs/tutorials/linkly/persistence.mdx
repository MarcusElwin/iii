...
description:
  "Move links out of worker memory into iii-state, then into a SQLite database for durable history."
owner: "devrel"
type: "tutorial"
---

So far the `link` worker keeps its links in an in-memory `Map`, so a restart loses every link. In
this chapter you move that data out of the worker: first into `iii-state` as a hot cache that
survives restarts, then into a `database` (SQLite) that holds the durable record of links and a
timestamped row for every click on a short code.
...
Now replace the in-memory store. Delete `link/src/store.ts` and add `link/src/codes.ts`, which holds
the code generator:
...
Then rewrite `link::create` and `link::resolve` in `link/src/index.ts` to read and write to the
`state` kv-store instead of the `Map`. In this code we set a scope `links` and store each code as a
`key`.

Replace the first part of `link/src/index.ts` with:
...
Give the service a moment to completely start and then run:
...
Add an `ensureSchema()` function that creates both tables on startup. The database worker accepts
SQL through its `database::execute` function:
...
The state write and log line stay; the database call is new in front of them:
...
Update `http::redirect` so it inserts a timestamped row into `clicks` before returning the redirect.
(The next chapter moves that write off this hot path onto a queue.) The resolve + 404 + 302 are from
Chapter 1; the insert is new:
...
Finally, call `ensureSchema()` once when the worker starts, replacing the existing `logger.info`
