...
`link-worker/src/codes.ts` with just the code generator:
...
```bash
iii trigger database::query --json '{"db":"primary","sql":"SELECT COUNT(*) AS clicks FROM clicks WHERE code = ?","params":["iii"]}'
```
...
[React to clicks](/tutorials/linkly/events), you move click recording onto a queue so redirects stay
fast.
