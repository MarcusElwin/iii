...
`iii-state` defaults to an in-memory store, which means restarting the engine clears every link.
That's fine for this chapter; [Ch. 3: Persist everything](/tutorials/linkly/persistence) swaps in
durable storage.
...
`link::create` and `link::resolve` store and look up the mapping by calling `state::set` and
`state::get` on the `iii-state` worker through `worker.trigger`. Worker-to-worker calls always flow
through the engine. The `link` worker doesn't import anything from `iii-state`; it just knows the
function names.
...
The link sits in `iii-state` now, but `GET /s/demo` has nowhere to go yet. There's no handler;
add one next.
...
the engine still clears every link, though: `iii-state` is in-memory until Chapter 3 swaps it for
durable storage.
