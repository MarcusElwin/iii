...
A project is a directory with a `config.yaml` that lists the workers your iii setup runs. Create one:
...
<Info>
  iii runs a worker for you from these `scripts`, but a worker is an ordinary service: you can
...
`iii worker init` scaffolds the worker but leaves your project untouched. iii starts the workers
...
<Check>
  You have a working domain worker. `link::create` and `link::resolve` are registered with the engine
  and callable from anywhere on the bus. Next you put them behind HTTP.
</Check>
