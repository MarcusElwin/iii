...
In Chapter 3 every redirect inserts a timestamped row into the `clicks` table inline, so a slow write
slows the redirect. In this chapter you move that insert onto a **queue** so redirects return
immediately, then use **pub/sub** to broadcast link events to independent subscribers: a Python
analytics worker and a cache refresher, neither of which the publisher knows about.
...
worker (already in your project from `iii project init`):
...
Then change `http::redirect` to enqueue the click row instead of writing it inline:

```typescript src/index.ts
  // Enqueue the row and return right away; the consumer writes it later.
  await worker.trigger({
...
The redirect now returns as soon as the row is accepted onto the queue. `link::record_click` drains
...
updates the database row and publishes a `link.updated` event:
...
[Ch. 5: Schedule and stream](/tutorials/linkly/scheduling-and-streams), you sweep expired links on a
schedule and stream live clicks to the browser.
