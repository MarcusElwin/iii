...
class TestValidate:
    def test_ok_when_next_block_and_folder_present(self, tmp_path):
        assert validate(_ready_docs(tmp_path), "0.17.0") == 0

    def test_ok_regardless_of_release_version(self, tmp_path):
        assert validate(_ready_docs(tmp_path), "1.0.0") == 0

    def test_ok_for_dry_run_version(self, tmp_path):
        assert validate(_ready_docs(tmp_path), "0.17.0-dry-run.1") == 0

    def test_fails_when_folder_missing(self, tmp_path):
        docs = _ready_docs(tmp_path)
        import shutil as _sh
        _sh.rmtree(docs / "next")
        assert validate(docs, "0.17.0") == 1
