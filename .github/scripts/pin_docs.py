...
Two subcommands:
* ``validate`` — gate a minor/major stable release on the Next docs being
  ready (a Next block exists and docs/next/ is non-empty). Runs even on dry runs.
* ``rotate`` — perform the folder moves and docs.json rewrite.
...
def validate(docs_dir: Path, version: str) -> int:
    ...
    release version is not compared to the Next label — rotation always pulls
    whatever is in next/.
    """
    try:
        ...
        print(f"::error::Cannot validate docs for {version}: {exc}")
        return 1

    folder = docs_dir / NEXT_DIR
    if not folder.is_dir() or not any(folder.iterdir()):
        print(
            f"::error::Missing Next docs at {folder} for release {version}. "
            f"Prepare docs/{NEXT_DIR}/ before tagging."
        )
        return 1

    print(f"pin_docs: Next docs ready for release {version}")
    return 0
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "validate":
        return validate(args.docs_dir, args.version)
